...
\section{Visual Car Detection} % (fold)
...
\subfloat[]{\includegraphics[width=0.4\textwidth]{pictures/hog_hist.pdf}\label{subfig:histogram}}
\caption{This figures shows gradient orientations for every pixel of the detection window \subref{subfig:gradient}, the corresponding histogram \subref{subfig:histogram} and its visualization as a glyph \subref{subfig:hog}. The directions of the arrows in \subref{subfig:histogram} represent the angle covered by each bin, the lengths of arrows correspond to the magnitude of the gradients that fall into the corresponding bin. In this example, for better visualization, the histogram consists of 5 values, covering the range of $180^\circ$, meaning that each bin covers the angle of $36^\circ$, whereas the experiments in this thesis, use the histograms consisting of 9 bins.}
...
\begin{figure}[b]
 \centering
\subfloat[]{\includegraphics[width=0.27\textwidth]{pictures/original.jpg}\label{subfig:original}} \hspace{3mm}
\subfloat[]{\includegraphics[width=0.27\textwidth]{pictures/glyph.jpg}\label{subfig:glyph}} \hspace{3mm}
\subfloat[]{\includegraphics[width=0.27\textwidth]{pictures/ihog.jpg}\label{subfig:ihog}}
\caption{This figure shows a car image \subref{subfig:original}, a representation of the histogram values using glyphs (similar to Fig.~\ref{subfig:hog}) \subref{subfig:glyph} and a HOG inversion image via HOGles~\cite{vondrick2013hoggles}~\subref{subfig:ihog}. The shadows around the car have a high influence on the corresponding HOG. Such details are smoothed out via learning the descriptor from the training data.}
\label{fig:hog_car}
\end{figure}
...
\section{Parking Lot Modeling} % (fold)
\label{sec:model}
...
\begin{figure}[t]%
\centering
\subfloat[Occupancy grid update of the camera field of view between the margin points marked with red circles containing two detected cars seen in black.]{\includegraphics[width=0.30\textwidth]{pictures/testmap.png}\label{fig:maptest}}\hspace{5mm}
...
\subsection{Occupancy Inference} % (fold)
...
% subsubsection occupancy_inference (end)
...
\subsection{Parking Area as a Graph} % (fold)
...
% subsection parking_area_as_a_graph_ (end)

\subsection{MDP Definition} % (fold)
...
% subsection actions (end)


\subsubsection{Transition Function} % (fold)
...
% subsubsection transition_function (end)
\subsubsection{Expected Time Minimization to Rewards Maximization} % (fold)
...
% subsubsection expected_time_to_rewards (end)
\subsubsection{Failure Costs} % (fold)
...
% subsubsection failure_costs (end)

\subsubsection{Success Rewards} % (fold)
...
To express $r_s$, we define $t_{\mathrm{walk}}^{s,s_{\mathrm{goal}}}$
...
% subsubsection success_rewards (end)
% subsection mdp_definition (end)
\subsection{Solution to the MDP} % (fold)
...
However, as time goes to infinity the sum in~\eqref{eq:non_discount_utility}
...
% subsection solution_to_the_mdp (end)
\subsection{Re-planning} % (fold)
